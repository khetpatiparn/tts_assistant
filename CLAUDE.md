...
2. **ต้องรันแอปดู UI จริง — แยกทุกอย่างออกจากของผู้ใช้:**
   ...
   **การรัน dev server แบบนี้อาจทำให้ `tsconfig.json` โดนแก้ transient เอง** (Next.js auto-patch compiler options ตอน dev ครั้งแรก) — เช็ก `git diff tsconfig.json` ก่อน commit เสมอ ถ้าเปลี่ยนให้ `git checkout -- tsconfig.json` (repo นี้ pin `target: "ES2017"` ไว้ตั้งใจ ดูหัวข้อ Gemini/BigInt ด้านล่าง)
...
- **ภาพ + label ผูกกันด้วยการ interleave** (`lib/gemini.ts` `generateTenPartPrompt`) — แต่ละรูปถูกส่งตามหลัง caption ของตัวเองทันที (ทีละคู่) ไม่ใช่ส่ง label รวมเป็นก้อนเดียวแล้วตามด้วยรูปทั้งหมด กันโมเดลเดาโดยนับตำแหน่งผิด (เคยพังกับสินค้าที่มีโครงสร้างไม่สมมาตร เช่น logo อยู่ด้านเดียว)
...
- **`lib/csv.ts`** เป็นที่เดียวที่มี `parseCsvRows`/`parseMonthDayLabel` ใช้ร่วมกันโดย `lib/clip-metrics.ts` (Content.csv) และ `lib/follower-activity.ts` (FollowerActivity.csv) — parser ไฟล์ CSV ใหม่จาก TikTok Studio ควร import จากตรงนี้ ไม่ต้องเขียน BOM-strip/quoted-field parser ซ้ำ (xlsx ยังใช้ `lib/affiliate.ts` แยกต่างหากเพราะเป็นคนละ format)
