...
- signal ที่ออกมาต้องตรงกับที่คำนวณมือไว้จากข้อมูลจริง (medianViews ≈ 1,741 · channelConv ≈ 0.19%):
  - **กระปุกกรองน้ำมัน** (conv 0.289% = 1.5× ช่อง, วิว 1,038 < median, 3 ออเดอร์) → `hidden-gem`
  - **ที่ลับเล็บแมว คลิปที่ 2** (conv 0.407%, วิว 1,476 < median, 6 ออเดอร์) → `hidden-gem`
  - **ชามสแตนเลส** (conv 0.067% ต่ำจริง แต่วิว 1,494 ไม่ถึง 2× median = 3,482) → ไม่ติด `reach-no-convert`
  - **ชั้นวางของในครัว** → ไม่มีข้อมูลวิวใน `Content.csv` เลย จึง**ไม่ถูกประเมิน** และต้องถูกนับใน
    ตัวเลข "คลิปที่ยังไม่มีข้อมูลวิว" แทน
  - import ครั้งแรก (snapshot เดียว) คาดว่าได้ **2 signal เป็น `hidden-gem` ทั้งคู่** ส่วน
    `emerging`/`fading` ต้องรอ import รอบที่ 2
